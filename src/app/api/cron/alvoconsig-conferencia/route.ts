...
export async function GET(request: NextRequest) {
  ...
  const { data: contatos, error } = await admin
    .from('crm_contatos')
    .select('id, wesales_contact_id, cpf, nome, telefone, convenio_id, matricula, margem_novo, margem_cartao_rmc, margem_cartao_rcc, refin_troco')
    .is('deleted_at', null)
  ...
  for (const local of contatos) {
    ...
    const refinTrocoRemoto = refinTrocoField ? parseMoneyField(customFieldValue(remoto, refinTrocoField.id)) : local.refin_troco
    ...
    const refin = refinTrocoRemoto
      ? {
          troco: refinTrocoRemoto,
          parcela: refinParcelaField ? parseMoneyField(customFieldValue(remoto, refinParcelaField.id)) : null,
          prazo: refinPrazoField ? Number.parseInt(customFieldValue(remoto, refinPrazoField.id) || '', 10) || null : null,
          taxa: refinTaxaField ? customFieldValue(remoto, refinTaxaField.id) : null,
        }
      : null
    const ofertas = await calcularOfertas(admin, convenioRemoto, margens, refin)
