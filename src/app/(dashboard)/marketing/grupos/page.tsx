...
import { listarGrupos, salvarGrupo, setGrupoStatus, type Grupo } from '@/lib/marketing/taxonomia-actions'
...
export default function GruposPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      else setMessage({ type: 'error', text: res.error || 'Erro ao carregar os grupos.' })
    } catch (error: any) {
      setMessage({ type: 'error', text: error?.message || 'Erro ao carregar os grupos.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setMessage(null)
    try {
      const res = await salvarGrupo({ id: editing.id, nome: String(editing.nome || '') })
      if (res.success) {
        setIsModalOpen(false)
        setEditing(null)
        setMessage({ type: 'success', text: editing.id ? 'Grupo atualizado.' : 'Grupo criado.' })
        await loadData()
      } else {
        setMessage({ type: 'error', text: res.error || 'Erro ao salvar o grupo.' })
      }
    } catch (error: any) {
      setMessage({ type: 'error', text: error?.message || 'Erro ao salvar o grupo.' })
    } finally {
      setSaving(false)
    }
  }

  async function handleToggle(item: Grupo) {
    setBusyId(item.id)
    setMessage(null)
    try {
      const nextActive = !item.is_active
      const res = await setGrupoStatus(item.id, nextActive)
      if (res.success) {
        setMessage({ type: 'success', text: nextActive ? 'Grupo reativado.' : 'Grupo inativado.' })
        await loadData()
      } else {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'flex-start', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <div>
          ...
          <div style={{ color: 'var(--brs-gray-500)', fontSize: '0.9rem', marginTop: '0.25rem' }}>
            Grupos de destino das artes (WhatsApp, Instagram, Facebook, TikTok…).
          </div>
        </div>
                ...
                <tr>
                  <td colSpan={3} style={{ textAlign: 'center', padding: '3rem' }}>
                    <div className="empty-state">
                      ...
                      <p>Cadastre o primeiro grupo de destino das artes.</p>
                    </div>
                  </td>
                </tr>
              ) : (
          ...
          <div className="modal" style={{ maxWidth: 480 }} onClick={(e) => e.stopPropagation()}>
            <form onSubmit={handleSave}>
              <div className="modal-header">
                <h3 className="modal-title">{editing?.id ? 'Editar Grupo' : 'Novo Grupo'}</h3>
              ...
              <div className="modal-body">
                <div className="form-group">
                  ...
                  <input
                    ...
                    className="form-control"
                    required
                    placeholder="Ex.: WhatsApp, Instagram, Facebook…"
                    value={editing?.nome || ''}
