...
import { getEsferas, salvarEsfera, setEsferaStatus, type Esfera } from '../cadastros-actions'

type FeedbackMessage = { type: 'success' | 'error'; text: string }

export default function EsferasPage() {
  const [items, setItems] = useState<Esfera[]>([])
  const [loading, setLoading] = useState(true)
  const [busyId, setBusyId] = useState<string | null>(null)
  const [message, setMessage] = useState<FeedbackMessage | null>(null)
  const [searchQuery, setSearchQuery] = useState('')
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editing, setEditing] = useState<Partial<Esfera> | null>(null)
  const [saving, setSaving] = useState(false)

  async function loadData() {
    setLoading(true)
    try {
      const res = await getEsferas()
      if (res.success) setItems(res.items || [])
      else setMessage({ type: 'error', text: res.error || 'Erro ao carregar as esferas.' })
    } catch (error: any) {
      setMessage({ type: 'error', text: error?.message || 'Erro ao carregar as esferas.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setMessage(null)
    try {
      const res = await salvarEsfera({ id: editing.id, nome: String(editing.nome || '') })
      if (res.success) {
        setIsModalOpen(false)
        setEditing(null)
        setMessage({ type: 'success', text: editing.id ? 'Esfera atualizada.' : 'Esfera criada.' })
        await loadData()
      } else {
        setMessage({ type: 'error', text: res.error || 'Erro ao salvar a esfera.' })
      }
    } catch (error: any) {
      setMessage({ type: 'error', text: error?.message || 'Erro ao salvar a esfera.' })
    } finally {
      setSaving(false)
    }
  }

  async function handleToggle(item: Esfera) {
    setBusyId(item.id)
    setMessage(null)
    try {
      const nextActive = !item.is_active
      const res = await setEsferaStatus(item.id, nextActive)
      if (res.success) {
        setMessage({ type: 'success', text: nextActive ? 'Esfera reativada.' : 'Esfera inativada.' })
        await loadData()
      } else {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', alignItems: 'flex-start', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <div>
          ...
          <div style={{ color: 'var(--brs-gray-500)', fontSize: '0.9rem', marginTop: '0.25rem' }}>
            Esferas de convênio (Municipal, Estadual, Federal…). Usadas no cadastro de Tipo de Convênio.
          </div>
        </div>
                ...
                <tr>
                  <td colSpan={3} style={{ textAlign: 'center', padding: '3rem' }}>
                    <div className="empty-state">
                      ...
                      <h3>Nenhuma esfera encontrada</h3>
                      <p>Cadastre a primeira esfera (Municipal, Estadual, Federal…).</p>
                    </div>
                  </td>
                </tr>
              ) : (
          ...
          <div className="modal" style={{ maxWidth: 480 }} onClick={(e) => e.stopPropagation()}>
            <form onSubmit={handleSave}>
              <div className="modal-header">
                <h3 className="modal-title">{editing?.id ? 'Editar Esfera' : 'Nova Esfera'}</h3>
