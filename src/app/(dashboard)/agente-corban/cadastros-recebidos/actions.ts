...
export async function reenviarLinkRascunho(rascunhoId: string): Promise<{ success: true } | { success: false; error: string }> {
  try {
    ...
    // Mesmo template do portal (fatia 5); o renderizador escapa o nome no HTML.
    const tpl = await obterMensagem('portal_link_retomada', { primeiro_nome: nome.split(/\s+/)[0] || 'tudo bem', url })
    const envio = await enviarEmailOnboarding({ to: r.email, subject: tpl.assunto, html: tpl.html })
